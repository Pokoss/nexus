...
function DashboardVillageScreen({villages,districts}) {
     const { data, setData, processing, post, reset, errors } = useForm();
        ...
        function editTheVillage(village, parish, subcounty, county,district) {
        handleOpenEdit("xl")
        ...
        const columns = [
            ...
            {
                selector: row => <button onClick={() => editTheVillage(row.village, row.parish.parish, row.parish.subcounty.subcounty,row.parish.subcounty.county.county,row.parish.subcounty.county.district.district, row.id)} className='bg-green-600 rounded-md p-1'><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="white" className="w-5 h-5">
                ...
                >
                    ...
                    >
                        <DialogBody divider className="grid place-items-center gap-4">
                            ...
                            />
                            <Input color='deep-orange' label='Parish' disabled
                                value={editParish} onChange={(event) => setEditParish(event.target.value)} size='sm'
                            />
                            <Input color='deep-orange' label='Village'
                                value={editVillage} onChange={(event) => setEditVillage(event.target.value)} size='sm'
                            />
...
export default DashboardVillageScreen